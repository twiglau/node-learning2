...
class Tools extends Controller {
  ...
  async init() {
    ...
    for(const activity of activityInfos) {
      // 循环处理导入
      let actInfo = activity['activity_info'];
      ...
      // 插入票信息
      let ticketInfo = activity['ticket_info'];
      ...
      // 导入券码列表
      const ret = await codeService.import(actId, ticketId, activity['code_list']);
      allRet = allRet && ret;
    }
