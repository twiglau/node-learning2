...
class ActService extends Service {
  ...
  async getList(page=0) {
    ...
    actList = await actModel.getOnlineList(page);
    if(!actList) {
      return [];
    }
    cache.set(ACT_DETAIL_CACHE_KEY.replace('{page}', page), actList, CACHE_TIME_SEC).then();
  ...
  async cacheList(cacheSize = 5) {
    ...
    for(let i=0; i<cacheSize; i++) {
      ...
      let actList = await actModel.getOnlineList(i);
  ...
  async cacheDetail(actInfo) {
    ...
    let ret = await cache.get(ACT_DETAIL_CACHE_KEY.replace('{actId}',actInfo['_id']), actInfo, CACHE_TIME_SEC, true);
    if(!ret) {
