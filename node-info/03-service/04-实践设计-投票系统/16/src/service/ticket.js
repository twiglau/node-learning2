...
class TicketService extends Service {
  ...
  async getUserTicketList(page=0) {
    page = parseInt(page);
    ...
    const codeModel = load.loadModel(this.ctx, 'code');
    const codeInfos = await codeModel.queryTicketIds(codeIds);
  ...
  async cacheDetail(ticketInfo) {
    ...
    let ret = await cache.get(TICKET_DETAIL_CACHE_KEY.replace('{ticketId}', ticketInfo['_id']), ticketInfo, CACHE_TIME_SEC, true);
    if(!ret) {
